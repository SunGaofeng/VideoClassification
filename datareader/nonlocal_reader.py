...
def apply_resize(rgbdata, min_size, max_size):
    ...
    outdata = np.zeros((length, out_height, out_width, channel), dtype = rgbdata.dtype)
    for i in range(length):
        outdata[i] = cv2.resize(rgbdata[i], (out_width, out_height))
    return outdata

def crop_mirror_transform(rgbdata, mean, std, cropsize = 224, use_mirror = True, center_crop = False, spatial_pos = -1):
    ...
    outdata = np.zeros((channel, length, cropsize, cropsize), dtype = rgbdata.dtype)
    outdata[:, :, :, :] = rgbdata[:, :, h_off : h_off+cropsize, w_off : w_off+cropsize]
    # apply mirror
...
def make_reader(filelist, batch_size, sample_times, is_training, shuffle, **dataset_args):
    ...
    def reader():
        ...
        for line in fl:
            ...
            try:
                ...
                logger.info('Error when loading {}, just skip this file'.format(fn))
                continue
            # add prepocessing
...
def make_multi_reader(filelist, batch_size, sample_times, is_training, shuffle, **dataset_args):
    ...
    def queue_reader():
        ...
        for i in range(len(reader_lists)):
            ...
            p_list[i] = multiprocessing.Process(target = read_into_queue, args = (reader_list, queue))
            p_list[i].start()
