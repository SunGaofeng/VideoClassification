...
class NEXTVLAD(ModelBase):
    ...
    def get_config(self):
        # model params
        self.num_classes = self.get_model_config('num_classes')
        self.video_feature_size = self.get_model_config('video_feature_size')
        self.audio_feature_size = self.get_model_config('audio_feature_size')
        self.cluster_size = self.get_model_config('cluster_size')
        self.hidden_size = self.get_model_config('hidden_size')
        self.groups = self.get_model_config('groups')
        self.expansion = self.get_model_config('expansion')
        self.drop_rate = self.get_model_config('drop_rate')
        self.gating_reduction = self.get_model_config('gating_reduction')
        self.eigen_file = self.get_model_config('eigen_file')
        # training params
        self.base_learning_rate = self.get_train_config('base_learning_rate')
        self.lr_boundary_examples = self.get_train_config('lr_boundary_examples')
        self.max_iter = self.get_train_config('max_iter')
        self.learning_rate_decay = self.get_train_config('learning_rate_decay')
        self.l2_penalty = self.get_train_config('l2_penalty')
        self.gradient_clip_norm = self.get_train_config('gradient_clip_norm')
    ...
    def build_model(self):
        ...
        self.network_output = [predictions]
        self.loss_ = avg_cost
