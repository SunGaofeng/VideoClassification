...
class NeXtVLAD(object):
  ...
  """
  def __init__(self, feature_size, cluster_size, is_training = True, expansion = 2, groups = None, inputname = 'video'):
  ...
  def forward(self, input):
    input = fluid.layers.fc(input = input, size = self.expansion * self.feature_size,
                            act = None,
    ...
    attention = fluid.layers.fc(input = input, size = self.groups,
                                act = 'sigmoid',
    ...
    cluster_weights = fluid.layers.create_parameter(
                            shape = [self.expansion * self.feature_size, self.groups * self.cluster_size], 
                            dtype = input.dtype, 
    ...
    activation = fluid.layers.batch_norm(activation, 
                                         is_test = (not self.is_training))

    # reshape of activation
    activation = fluid.layers.reshape(activation, [-1, self.groups, self.cluster_size])
    ...
    a = fluid.layers.elementwise_mul(a_sum, cluster_weights2, axis = 1) # output shape [-1, self.cluster_size, feature_size]
    ...
    reshaped_input = fluid.layers.reshape(input, [-1, self.groups, feature_size])
    # mat multiply activation and reshaped_input
    vlad = fluid.layers.matmul(activation, reshaped_input) # output shape [-1, self.cluster_size, feature_size]
    ...
    vlad = fluid.layers.reshape(vlad, [-1, self.cluster_size * feature_size])
...
class NeXtVLADModel(object):
  ...
  def create_model(self,
    ...
    audio_nextvlad = NeXtVLAD(128, cluster_size, is_training, expansion = expansion, groups = groups, inputname = 'audio')
    ...
    # drop out
    if drop_rate > 0.:
      vlad = fluid.layers.dropout(vlad, drop_rate, is_test = (not is_training))

    # add fc
    activation = fluid.layers.fc(input = vlad, 
                                 size = hidden_size,
                                 act = None,
                                 name = 'hidden1_fc',
                                 param_attr = fluid.ParamAttr(
    ...
    # add fc, gate 1
    gates = fluid.layers.fc(input = activation, 
                            ...
                            param_attr = fluid.ParamAttr(
                                            ...
                                            initializer = fluid.initializer.MSRA(uniform=False)),
                            bias_attr = False)
    gates = fluid.layers.batch_norm(gates, 
                                    is_test = (not is_training), 
                                    act = 'relu')

    # add fc, gate 2
    gates = fluid.layers.fc(input = gates, 
                            size = hidden_size,
                            act = 'sigmoid',
                            name = 'gating_fc2',
                            param_attr = fluid.ParamAttr(
